...
export default function TargetIgnorePaths() {
    const { register, control } = useFormContext()
    const [totalTarget, setTotalTarget] = useState([1])
    const { remove } = useFieldArray({
        control,
        name: `TargetSettings.TargetIgnorePaths`
                        ...
                        <div className="col-sm-10">
                            <div class="col-auto">
                                <div class="form-floating mb-3">
                                    <input type="text" class="form-control" id={i} placeholder="TargetIgnorePaths" name="TargetIgnorePaths" {...register(`TargetSettings.TargetIgnorePaths.${i}`, { required: true })} />
                                    <label for="floatingInput">Target Ignore Paths</label>
                                </div>
                            </div>
                        </div>
